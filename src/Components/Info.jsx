...
const Info = () => {
  const infoData = [
    ...
    },
    {
      ...
      description: "Stream on any device, anywhere with Netflix.",
      icon: (
        <svg width="72" height="72" viewBox="0 0 72 72" fill="none">
  ...
  return (
    <div className="min-h-[70vh] w-full px-4 sm:px-8 lg:px-1 sm:mb-6 mt-5">
      <h1 className="text-white font-semibold text-2xl sm:text-3xl pt-6 sm:pt-10">
        More Reasons to Join
      </h1>
      <div className="sm:flex sm:flex-wrap sm:gap-x-5 sm:gap-y-0 md:gap-4">
        {info.map((item, index) => (
          <div
            key={index}
            className="relative flex w-[85vw] h-[32vh] sm:w-[40vw] sm:h-[38vh] md:w-[20vw] md:h-[45vh] shadow-md hover:shadow-2xl transition-shadow duration-300 mt-4 sm:mt-12"
          >
            <div
              className="flex-shrink-0 w-full h-full bg-gradient-to-r from-[#1A2143] to-[#1E1528] rounded-2xl "
              style={{
                backgroundImage: `linear-gradient(135deg, [#1A2143], [#1E1528])`,
              }}
            >
              <div className="flex-grow py-6 px-4 text-white">
                <h2 className="font-semibold text-lg sm:text-2xl md:text-[1.8vw] leading-none sm:leading-8">{item.title}</h2>
                <p className="font-medium text-[3.6vw] sm:text-lg md:text-[1.2vw] text-[#A19EA7] mt-3 md:mt-6 leading-5 tracking-tight">{item.description}</p>
              </div>
              <div className="icon absolute bottom-2 right-3">
                <div>{item.icon}</div>
              </div>
            </div>
          </div>
